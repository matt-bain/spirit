...
export var speed = 130.0 #the horizontal move speed of the bull
...
func reset():
	health = 100
	$Health.value = health #update healthbar node
		...
		match current_state:
			state.moving: #bull is moving
				if target && !target.frozen: #if has a target and target is not frozen
					...
					if distance <= 82: #if the player is within punching distance
...
func hit(damage):
	if !dead: #only apply damage if bull is not dead
		...
		$Health.value = health #update healthbar
		$HitSound.pitch_scale = rand_range(0.8, 1.2)
		if !$HitSound.playing:
			$HitSound.play()

#returns true if the entity is a fire projectile
func body_is_projectile(body):
	return body.get_name().find("ProjectileFire") != -1 && active
...
#called when a bull animation ends
func _on_Animation_animation_finished():
	#if an attack animation just finished
	if $Animation.animation == "attack" || $Animation.animation == "attack_up" || $Animation.animation == "attack_down" :
		$Animation.frame = 0 #reset the animation
		current_state = state.moving #move the bull again
