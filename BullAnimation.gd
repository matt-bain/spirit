	...
	if animation == "attack" || animation == "attack_up" || animation == "attack_down":
		...
		if frame == 2 || frame == 3 || frame == 4:
			if currenthit && !has_been_hit: #if there is an unhit target
				...
				if currenthit.get_name() == "Human": #is the target a human?
					if !currenthit.first_boss_death: #target is a human and has not died yet
						currenthit.first_boss_death = true #target has now died to the boss
						currenthit.emit_signal("damage_taken", 100) #apply 100 damage to the player (instant kill)
					else: #target has died prior
						currenthit.emit_signal("damage_taken", 50) #apply 50 damage to the player
				else:
